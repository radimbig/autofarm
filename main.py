...
token = "YOUR_TOKEN";  #HERE IS YOUR_TOKEN
url ="https://api.telegram.org/bot" + token + "/sendMessage?chat_id=YOUR_CHAT_ID&text=";  #YOUR_CHAT_ID
...
def sborPlus(variant):
    ...
    while True:
        ...
        time.sleep(1)
        while True:
            chestLoc = pyautogui.locateOnScreen(chest, confidence=0.5)
            
            if chestLoc != None:
                pyautogui.click(chestLoc)
                break
            else:
                tgsend("Something wrong... No chest")
                print("Stopping...")
                return 0
        time.sleep(1)
